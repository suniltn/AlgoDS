...
public class Substitute {
    public int getValue(String key, String code) {
        HashMap<Character, Integer> lookupTable = createLookupTable(key);
        StringBuilder result = new StringBuilder();
        char[] codeArray = code.toCharArray();
        for (char c : codeArray) {
            if (lookupTable.containsKey(c)) {
            	result.append(lookupTable.get(c));
            }
        }
    ...
    private HashMap<Character, Integer> createLookupTable(String key) {
