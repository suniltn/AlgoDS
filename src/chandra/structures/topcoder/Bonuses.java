...
public class Bonuses {
	public int[] getDivision(int[] points) {
		...
		boolean[] extraBonuses = new boolean[points.length];
		int totalPoints = 0;
		int remainingBonus = 100;
		for (int point : points)
			totalPoints += point;
		
		for (int i=0;i<points.length;i++) {
			bonuses[i] = (points[i]*100)/totalPoints;
			remainingBonus -= bonuses[i];
		}
		...
		while(remainingBonus > 0) {
			...
			int maxPoints = -1;
			for (int i=0;i<points.length;i++)
			if (!extraBonuses[i] && points[i]>maxPoints) {
				employee = i;
				maxPoints = points[i];
			}
			extraBonuses[employee] = true;
			bonuses[employee] += 1;
			remainingBonus -= 1;
		}
		return bonuses;
	...
	public static void main(String[] args) {
		...
		int[] bonusPoints = {5,5,5,5,5,5};
		int[] result = b.getDivision(bonusPoints);
		for (int r : result) {
